...
const Coins = () => {
  ...
  const handleBuy = (id: number) => buyCoin(id);
  const handleSell = (id: number) => sellCoin(id);

  const showItems = view === 'market'
    ? coinsItems.filter(item => !ownedCoins.includes(item.id))
    : coinsItems.filter(item => ownedCoins.includes(item.id));
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">🪙 Coin Collection</h1>
