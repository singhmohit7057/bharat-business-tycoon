...
const Harbor = () => {
  ...
  const handleBuy = (id: number) => buyShip(id);
  const handleSell = (id: number) => sellShip(id);

  const showItems = view === 'market'
    ? harborItems.filter(item => !ownedShips.includes(item.id))
    : harborItems.filter(item => ownedShips.includes(item.id));
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">Harbor</h1>
