...
const Jewels = () => {
  ...
  const handleBuy = (id: number) => buyJewel(id);
  const handleSell = (id: number) => sellJewel(id);

  const showItems =
    view === 'market'
      ? jewelsItems.filter((item) => !ownedJewels.includes(item.id))
      : jewelsItems.filter((item) => ownedJewels.includes(item.id));
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">Jewels</h1>
