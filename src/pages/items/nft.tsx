...
const NFT = () => {
  ...
  const handleBuy = (id: number) => buyNft(id);
  const handleSell = (id: number) => sellNft(id);

  const showItems =
    view === 'market'
      ? nftItems.filter((item) => !ownedNfts.includes(item.id))
      : nftItems.filter((item) => ownedNfts.includes(item.id));
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">NFT Collection</h1>
