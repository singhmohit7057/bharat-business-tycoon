...
const UniqueItems = () => {
  ...
  const handleBuy = (id: number) => buyUnique(id);
  const handleSell = (id: number) => sellUnique(id);

  const showItems =
    view === 'market'
      ? uniqueItems.filter((item) => !ownedUnique.includes(item.id))
      : uniqueItems.filter((item) => ownedUnique.includes(item.id));
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">Unique Items</h1>
