...
const Hanger = () => {
  ...
  const handleBuy = (id: number) => buyPlane(id);
  const handleSell = (id: number) => sellPlane(id);

  const showItems = view === 'market'
    ? hangerItems.filter(item => !ownedPlanes.includes(item.id))
    : hangerItems.filter(item => ownedPlanes.includes(item.id));
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">Hanger</h1>
