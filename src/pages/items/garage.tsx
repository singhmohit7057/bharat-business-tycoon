...
const Garage = () => {
  const isClient = useIsClient(); // 👈 check if it's client-side
  ...
  const handleBuy = (id: number) => buyCar(id);
  const handleSell = (id: number) => sellCar(id);

  const showItems = view === 'market'
    ? garageItems.filter(item => !ownedCars.includes(item.id))
    : garageItems.filter(item => ownedCars.includes(item.id));

    // Prevent mismatch by rendering only after client-side mount
  if (!isClient) return null;
  
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">Garage</h1>
