...
const Stamps = () => {
  ...
  const handleBuy = (id: number) => buyStamp(id);
  const handleSell = (id: number) => sellStamp(id);

  const showItems =
    view === 'market'
      ? stampsItems.filter((item) => !ownedStamps.includes(item.id))
      : stampsItems.filter((item) => ownedStamps.includes(item.id));
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">Stamps</h1>
