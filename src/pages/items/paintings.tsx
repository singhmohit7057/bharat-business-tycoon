...
const Paintings = () => {
  ...
  const handleBuy = (id: number) => buyPainting(id);
  const handleSell = (id: number) => sellPainting(id);

  const showItems =
    view === 'market'
      ? paintingsItems.filter((item) => !ownedPaintings.includes(item.id))
      : paintingsItems.filter((item) => ownedPaintings.includes(item.id));
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">Paintings Gallery</h1>
