...
const Islands = () => {
  ...
  const handleBuy = (id: number) => buyIsland(id);
  const handleSell = (id: number) => sellIsland(id);

  const showItems = view === 'market'
    ? islandItems.filter(item => !ownedIslands.includes(item.id))
    : islandItems.filter(item => ownedIslands.includes(item.id));
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-2">🏝️ Islands</h1>
